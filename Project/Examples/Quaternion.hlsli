...
float4 q_conj(float4 q) { return float4(-q.x, -q.y, -q.z, q.w); }
...
float4 q_diff(float4 q1, float4 q2) { return q2 * q_inverse(q1); }
...
float4 q_slerp(in float4 a, in float4 b, float t) {
    ...
    float4 result =
        float4(blendA * a.xyz + blendB * b.xyz, blendA * a.w + blendB * b.w);
...
float4x4 quaternion_to_matrix(float4 quat) {
    float4x4 m = float4x4(float4(0, 0, 0, 0), float4(0, 0, 0, 0),
