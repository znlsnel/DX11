...
hlab::FoliageModel::FoliageModel(ComPtr<ID3D11Device> &device,
    ...
    MakeBoundingBox(meshes); 
}
 
void hlab::FoliageModel::MakeBoundingBox(const vector<MeshData> &meshDatas) {
...
void hlab::FoliageModel::GetMeshInFrustum(vector<shared_ptr<Mesh>> &mesh) {
...
std::queue<pair<BVNode, int>> queue;
    ...
    while (!queue.empty()) {
       ...
       // Matrix t = m_worldRow;
        ...
        if (check) {
            if (node.objectID >= 0) {
                mesh.push_back(m_meshes[m_meshStartID[node.objectID]]);
            }
...
void hlab::FoliageModel::Render(ComPtr<ID3D11DeviceContext> &context) {
    ...
    static int searchingMeshTimer = 0;
    if (searchingMeshTimer > 10) {
        GetMeshInFrustum(m_foundMesh);
        searchingMeshTimer = 0;
    }
    searchingMeshTimer++;  
     cout << "m_foundMesh.size : "<<  m_foundMesh.size()
    << "\n";
    if (m_isVisible) {
        for (const auto &mesh : m_foundMesh) {
...
//GraphicsPSO &hlab::FoliageModel::GetPSO(const bool wired) {
//    // TODO: 여기에 return 문을 삽입합니다.
//}
//
//GraphicsPSO &hlab::FoliageModel::GetDepthOnlyPSO() {
//    // TODO: 여기에 return 문을 삽입합니다.
//}
//
//GraphicsPSO &hlab::FoliageModel::GetReflectPSO(const bool wired) {
//    // TODO: 여기에 return 문을 삽입합니다. 
//}
//  
