...
PixelShaderOutput main(PixelShaderInput input)
{
    ...
    float3 albedo = useAlbedoMap ? albedoTex.SampleLevel(linearSampler, input.texcoord, lod).rgb 
                                 : material.albedo;
    float ao = useAOMap ? aoTex.SampleLevel(linearSampler, input.texcoord, lod).r : 1.0;
    float metallic = useMetallicMap ? metallicRoughnessTex.SampleLevel(linearSampler, input.texcoord, lod).b
                                    : material.metallic;
    float roughness = useRoughnessMap ? metallicRoughnessTex.SampleLevel(linearSampler, input.texcoord, lod).g 
                                      : material.roughness;

    float3 emission = useEmissiveMap ? emissiveTex.SampleLevel(linearSampler, input.texcoord, lod).rgb 
                                     : material.emission;
