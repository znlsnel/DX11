...
class BasicMeshGroup {
  public:
    ...
    BasicMeshGroup(ComPtr<ID3D11Device> &device,
                   ...
                   const std::string &basePath, const std::string &filename, bool invertNormals);
    BasicMeshGroup(ComPtr<ID3D11Device> &device,
                   ComPtr<ID3D11DeviceContext> &context,
  ...
  public:
    ...
    ComPtr<ID3D11ShaderResourceView> m_shadowSRV; // <- 그림자에 사용
    ComPtr<ID3D11Buffer> m_lightConstBuffer;      // <- 그림자에 사용
  ...
  private:
    ...
    ComPtr<ID3D11SamplerState> m_borderSamplerState;
