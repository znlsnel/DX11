...
// Example example = std::make_unique<Example>
...
int main() {
      ...
      example = std::make_unique<Example>(hwnd, width, height);
      ...
      while (WM_QUIT != msg.message) {
            if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
                  ...
                  //	 Start
                  // the Dear ImGui frame ImGui_ImplDX11_NewFrame();
                  // ImGui_ImplWin32_NewFrame();
                  // ImGui::NewFrame();
                  // ImGui::Begin("Object Control Box");
                  //
                  // ImGui::End();
                  // ImGui::Render();
...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
      ...
      case WM_SYSCOMMAND:
            ...
            break;
      case WM_MOUSEMOVE:
            // std::cout << "Mouse " << LOWORD(lParam) << " " <<
            // HIWORD(lParam) << std::endl;
            break;
      case WM_LBUTTONUP: {
            // int xPos = GET_X_LPARAM(msg);
            // int yPos = GET_Y_LPARAM(msg);
